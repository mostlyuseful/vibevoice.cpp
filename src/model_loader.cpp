...
bool ModelLoader::load(const std::string& path) {
    ...
    //
    // On CPU backend we mmap the gguf file and point tensor data directly
    // into the mapped region, avoiding a full heap copy of the weights.
    // On GPU backends we fall through to the heap-staging path below.
    bool is_cpu = ggml_backend_is_cpu(vv::backend());
    if (n > 0 && is_cpu) {
    ...
    // For GPU backends: stream the on-disk tensor bytes into the backend
    // buffer. We open the file ourselves (gguf has the byte offsets but
    // doesn't expose a streaming reader) and read each tensor in turn
    // into a small CPU staging buffer, then upload via
    // ggml_backend_tensor_set.
    //
    // The CPU mmap path above skips this entirely — tensor data already
