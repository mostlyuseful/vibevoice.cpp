...
This branch is converging on a **KugelAudio-only TTS** publish surface.
The primary conversion workflow is therefore the current **KugelAudio v1 acceptance path**.
Legacy VibeVoice conversion notes remain below only as migration/reference material.

For the supported KugelAudio v1 workflow, the important pieces are:
- `scripts/convert_tokenizer.py`
- `scripts/convert_kugelaudio_to_gguf.py`
...
- quality target: `f16` transcript recall >= 95% of canonical with 0.80 floor,
  plus speaker similarity >= 95% of canonical with 0.60 floor
...
Legacy VibeVoice-only voice-cache flows are no longer part of the published
surface for this branch. Historical references are retained only where needed
to explain converter/runtime compatibility boundaries.
...
```
  ...
  │ microsoft/VibeVoice-…        │── safetensors    ─► convert_kugelaudio_to_gguf.py ─► …gguf
  └─────────────────────────────┘
                                                                 │
                                              (optional)          │
                                                                 ▼
...
supported TTS run; you only need to convert it once and reuse it across model
checkpoints.
...
Legacy ASR paths in this repo re-use three Qwen2.5 vision-token IDs to mark speech regions
in the prompt:
...
Walks a KugelAudio checkpoint directory (`config.json` + sharded
...
```bash
hf download kugelaudio/kugelaudio-0-open --local-dir models/kugelaudio-0-open
python scripts/convert_kugelaudio_to_gguf.py \
    --src models/kugelaudio-0-open \
    --out models/kugelaudio-f16.gguf
```

Checkpoint families are auto-detected from the config and tensor inventory.
Only `kugelaudio/kugelaudio-0-open` is part of the published acceptance path;
other rows below are maintained only as legacy/internal compatibility notes:

| Variant / checkpoint | Source repo | Detection / notes |
| --- | --- | --- |
| `realtime-0.5b` | `microsoft/VibeVoice-Realtime-0.5B` | legacy/internal pre-baked-voice compatibility path |
| `1.5b` | `microsoft/VibeVoice-1.5B` | legacy/internal raw-reference compatibility path |
| `asr-7b` | `microsoft/VibeVoice-ASR` | legacy/internal ASR compatibility path |
...
| --- | --- |
| `kugelaudio.checkpoint` | `vibevoice.variant` *(currently written as `kugelaudio-0-open`; runtime normalizes this onto the existing internal raw-reference compatibility branch)* |
...
- the current runtime branch normalization maps `kugelaudio-0-open` onto the
  existing internal raw-reference compatibility load path until a dedicated
  KugelAudio runtime variant exists
...
| `model.acoustic_tokenizer.decoder.…`     (all TTS paths, including raw-reference compatibility) | `at.dec.…`                                   |
| `model.semantic_tokenizer.…`             (ASR + raw-reference compatibility)                    | `st.…`                                       |
| `model.acoustic_connector.{linear1,norm,linear2}.…`                                             | `ac.{fc1,norm,fc2}.…`                        |
| `model.semantic_connector.…`             (ASR + raw-reference compatibility)                    | `sc.…`                                       |
| `model.prediction_head.…`                (all TTS paths, including raw-reference compatibility) | `dh.…`                                      |
| `model.tts_eos_classifier.…`             (legacy pre-baked-voice compatibility only)            | `eos.…`                                      |
| `lm_head.weight`                          (ASR + raw-reference compatibility; tied-embed synth for that path) | `lm_head.weight`                 |
...
## Removed legacy pre-baked voice conversion flow

The old `scripts/convert_voice_to_gguf.py` helper for realtime VibeVoice
voice-cache artifacts has been removed from the published surface of this
branch.

That flow depended on pre-baked `voice.gguf` conditioning, which is no longer
exposed by the published CLI. The supported and documented path is raw-reference
conditioning via `--ref-audio`.

## `scripts/quantize_gguf.py`

Optional. Takes a supported GGUF model and produces a quantized variant by
...
For finer-grained family mixes (e.g., keep core LM weights at `--type` and
put select preserved families to Q8_0/Q4_K/Q6_K), use the C++ quantizer.
It now streams tensor payloads one-at-a-time during rewrite, which reduces
peak RAM versus the older in-memory destination-buffer approach:

```bash
./build/bin/kugelaudio-quantize --help
```

It supports per-family overrides such as `--at-block-ffn-type`,
`--st-block-ffn-type`, `--dh-type`, `--embed-type`, `--ac-connector-type`,
and `--sc-connector-type`.
...
## Published acceptance verification

After conversion (and optional quantization), validate the supported bundle with
KugelAudio's published acceptance path:
...
The published harness runs canonical and ggml TTS, then evaluates both outputs
with external evaluators:
- `faster-whisper` for transcript recall
- SpeechBrain ECAPA-TDNN for speaker similarity

The acceptance threshold requires ggml to remain within 95% of canonical on
both metrics, with absolute floors of 0.80 transcript recall and 0.60 speaker
similarity.
