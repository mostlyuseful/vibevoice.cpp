...
int main() {
    ...
    if (!file_ok(model_path) || !file_ok(tok_path) || !file_ok(ref_wav)) {
        std::fprintf(stderr,
            "skip: CUDA smoke test needs KUGELAUDIO_Q8_MODEL + KUGELAUDIO_TOKENIZER + KUGELAUDIO_REF_WAV\n");
        return 77;
    }
    ...
    if (!vv::vibevoice_load(model_path, &model)) {
        std::fprintf(stderr, "FAIL: CUDA q8_0 load %s\n", model_path);
        return 1;
    }
    if (!model.loader.has_key("kugelaudio.architecture")) {
        std::fprintf(stderr, "FAIL: CUDA q8_0 model %s is not detected as KugelAudio\n", model_path);
        return 2;
    }
    ...
    if (rc != 0) {
        std::fprintf(stderr, "FAIL: CUDA q8_0 generate rc=%d\n", rc);
        return 5;
    }
    if (samples.size() < 512) {
        std::fprintf(stderr, "FAIL: CUDA q8_0 produced too few samples (%zu)\n", samples.size());
        return 6;
    }
    ...
    std::printf("kugelaudio_cuda_smoke: backend=%s samples=%zu rms=%.6f\n",
                vv::backend_name(), samples.size(), rms);
    if (!finite) {
        std::fprintf(stderr, "FAIL: CUDA q8_0 produced non-finite samples\n");
        return 7;
    }
    if (rms < 1e-5) {
        std::fprintf(stderr, "FAIL: CUDA q8_0 output is effectively silent (rms=%.8f)\n", rms);
        return 8;
    }
    return 0;
}
