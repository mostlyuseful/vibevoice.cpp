...
int main() {
    const char* ok_path = std::getenv("KUGELAUDIO_MODEL");
    const char* bad_schema_path = std::getenv("KUGELAUDIO_BAD_SCHEMA_MODEL");
    const char* bad_checkpoint_path = std::getenv("KUGELAUDIO_BAD_CHECKPOINT_MODEL");
    const char* missing_semantic_path = std::getenv("KUGELAUDIO_MISSING_SEMANTIC_MODEL");
    const char* missing_acoustic_path = std::getenv("KUGELAUDIO_MISSING_ACOUSTIC_MODEL");
    const char* missing_metadata_path = std::getenv("KUGELAUDIO_MISSING_METADATA_MODEL");
    if (!file_ok(ok_path) || !file_ok(bad_schema_path) || !file_ok(bad_checkpoint_path) ||
        ...
        std::fprintf(stderr,
                     "skip: set KUGELAUDIO_MODEL, "
                     "KUGELAUDIO_BAD_SCHEMA_MODEL, "
                     "KUGELAUDIO_BAD_CHECKPOINT_MODEL, "
                     "KUGELAUDIO_MISSING_SEMANTIC_MODEL, "
                     "KUGELAUDIO_MISSING_ACOUSTIC_MODEL and "
                     "KUGELAUDIO_MISSING_METADATA_MODEL\n");
        return 77;
    }
    ...
    if (g_last_log.find("detected kugelaudio checkpoint=kugelaudio-0-open") == std::string::npos ||
        g_last_log.find("runtime_path=kugelaudio_raw_ref_tts") == std::string::npos ||
