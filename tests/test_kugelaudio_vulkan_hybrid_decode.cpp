...
int main() {
    ...
    if (!file_ok(model_path) || !file_ok(tok_path) || !file_ok(ref_wav)) {
        std::fprintf(stderr,
            "skip: Vulkan hybrid decode test needs KUGELAUDIO_Q8_MODEL + KUGELAUDIO_TOKENIZER + KUGELAUDIO_REF_WAV\n");
        return 77;
    }
    ...
    p.seed                    = 123;
    p.verbose                 = false;
    p.final_decoder_backend   = vv::FinalDecoderBackend::Cpu;
    ...
    const int rc = vv::vibevoice_tts_generate(
        ...
        &samples);
    if (rc != 0) {
        std::fprintf(stderr, "FAIL: Vulkan hybrid decode generate rc=%d\n", rc);
        return 5;
    }
    if (samples.size() < 512) {
        std::fprintf(stderr, "FAIL: Vulkan hybrid decode produced too few samples (%zu)\n", samples.size());
        return 6;
    }
    ...
    std::printf("kugelaudio_vulkan_hybrid_decode: backend=%s samples=%zu rms=%.6f\n",
                vv::backend_name(), samples.size(), rms);
    if (!finite) {
        std::fprintf(stderr, "FAIL: Vulkan hybrid decode produced non-finite samples\n");
        return 7;
    }
    if (rms < 1e-5) {
        std::fprintf(stderr, "FAIL: Vulkan hybrid decode output is effectively silent (rms=%.8f)\n", rms);
        return 8;
    }
    return 0;
}
