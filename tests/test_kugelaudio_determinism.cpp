...
int main() {
    const char* model_path = std::getenv("VIBEVOICE_KUGELAUDIO_MODEL");
    ...
    if (!file_ok(model_path) || !file_ok(tok_path) || !file_ok(ref_wav)) {
        std::fprintf(stderr,
                     "skip: set VIBEVOICE_KUGELAUDIO_MODEL, VIBEVOICE_TOKENIZER, and VIBEVOICE_REF_WAV\n");
        return 77;
    }

    setenv("VIBEVOICE_BACKEND", "cpu", 1);
