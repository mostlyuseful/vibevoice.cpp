...
def main() -> int:
    ...
    if evaluators.get("transcription", {}).get("backend") != "faster-whisper":
        raise SystemExit("FAIL: acceptance config must pin faster-whisper transcription")
    if evaluators.get("speaker_similarity", {}).get("backend") != "speechbrain-ecapa":
        raise SystemExit("FAIL: acceptance config must pin speechbrain-ecapa speaker similarity")
