...
ALIASES_CANONICAL_TO_GGML = {
    ...
    "19_first_diffusion_noise": "frame_0000_diffusion_noise",
    "20_first_diffusion_latent": "18_first_diffusion_latent",
    "21_first_step_embed": "19_first_step_embed",
}
...
def compare_arrays(lhs: np.ndarray, rhs: np.ndarray) -> dict[str, Any]:
    ...
    special_match = (~np.isfinite(lhs_f)) & (~np.isfinite(rhs_f)) & (lhs_f == rhs_f)
    special_mismatch = int((~finite & ~special_match).sum())
    lhs_cmp = lhs_f[finite]
    rhs_cmp = rhs_f[finite]
    diff = np.abs(lhs_cmp - rhs_cmp)
    lhs_norm = np.linalg.norm(lhs_cmp)
    rhs_norm = np.linalg.norm(rhs_cmp)
    cosine = None
    if lhs_norm > 0 and rhs_norm > 0:
        cosine = float(np.dot(lhs_cmp, rhs_cmp) / (lhs_norm * rhs_norm))
    elif lhs_norm == 0 and rhs_norm == 0 and special_mismatch == 0:
        cosine = 1.0
    return {
        "shape_match": True,
        "lhs_shape": list(lhs.shape),
        ...
        "special_mismatch_count": special_mismatch,
        "max_abs_diff": float(diff.max(initial=0.0)),
        "mean_abs_diff": float(diff.mean() if diff.size else 0.0),
        "rmse": float(np.sqrt(np.mean((lhs_cmp - rhs_cmp) ** 2)) if diff.size else 0.0),
        "cosine": cosine,
    }
