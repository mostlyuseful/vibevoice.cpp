...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--dump-frames", type=int, default=1, help="number of generated diffusion frames to dump")
    ...
    p.add_argument("--device", default="auto", help="cpu, cuda, cuda:0, or auto")
    p.add_argument("--torch-dtype", choices=["float32", "float16", "bfloat16"], default="float32")
    return p.parse_args()
...
def frame_stage(frame_index: int, suffix: str) -> str:
    return f"frame_{frame_index:04d}_{suffix}"


def sample_speech_tokens_with_dump(model: Any,
                                   condition: torch.Tensor,
                                   neg_condition: torch.Tensor,
                                   cfg_scale: float,
                                   dump_dir: Path | None = None,
                                   frame_index: int | None = None) -> tuple[torch.Tensor, torch.Tensor]:
    """Mirror canonical sample_speech_tokens while exposing the initial noise."""
    model.model.noise_scheduler.set_timesteps(model.ddpm_inference_steps)
    if cfg_scale == 1.0:
        speech = torch.randn(condition.shape[0], model.config.acoustic_vae_dim).to(condition)
        initial_noise = speech.clone()
        for step_idx, t in enumerate(model.model.noise_scheduler.timesteps):
            ...
            if dump_dir is not None and frame_index is not None:
                dump_f32(dump_dir, frame_stage(frame_index, f"dpm_step_{step_idx:02d}_model_output"),
                         eps[0], "canonical raw diffusion-head model output before scheduler step")
            variance_noise = torch.randn(eps.shape, device=eps.device, dtype=torch.float32)
            if dump_dir is not None and frame_index is not None:
                dump_f32(dump_dir, frame_stage(frame_index, f"dpm_step_{step_idx:02d}_variance_noise"),
                         variance_noise[0], "canonical per-frame per-DPM-step SDE variance noise")
            speech = model.model.noise_scheduler.step(eps, t, speech, variance_noise=variance_noise).prev_sample
            if dump_dir is not None and frame_index is not None:
                dump_f32(dump_dir, frame_stage(frame_index, f"dpm_step_{step_idx:02d}_sample"),
                         speech[0], "canonical per-frame DPM sample after scheduler step")
        return speech, initial_noise

    combined_condition = torch.cat([condition, neg_condition], dim=0).to(
        model.model.prediction_head.device
    )
    speech = torch.randn(combined_condition.shape[0], model.config.acoustic_vae_dim).to(
        combined_condition
    )
    initial_noise = speech[: len(speech) // 2].clone()
    for step_idx, t in enumerate(model.model.noise_scheduler.timesteps):
        ...
        if dump_dir is not None and frame_index is not None:
            dump_f32(dump_dir, frame_stage(frame_index, f"dpm_step_{step_idx:02d}_sample"),
                     speech[0], "canonical per-frame DPM sample after scheduler step")
    return speech[: len(speech) // 2], initial_noise
...
def main() -> int:
    ...
    from kugelaudio_open.schedule.dpm_solver import DPMSolverMultistepScheduler
    ...
    # Transformers may construct modules under a meta-device context while
    # loading. KugelAudio's scheduler creates real tensors during __init__, so
    # force that scheduler init back onto CPU.
    _scheduler_init = DPMSolverMultistepScheduler.__init__
    def _scheduler_init_on_cpu(self: Any, *a: Any, **kw: Any) -> None:
        with torch.device("cpu"):
            _scheduler_init(self, *a, **kw)
    DPMSolverMultistepScheduler.__init__ = _scheduler_init_on_cpu

    # Avoid Transformers' meta-device low-memory construction here: KugelAudio's
    # scheduler creates concrete tensors during __init__, which cannot be copied
    # from meta tensors with newer torch/transformers combinations.
    ...
    with torch.no_grad():
        ...
        generated_diffusion_frames = 0
        pending_hidden_after_step_frame: int | None = None
        ...
        for _step in range(args.max_new_tokens):
            ...
            past_key_values = outputs.past_key_values
            if pending_hidden_after_step_frame is not None:
                if pending_hidden_after_step_frame < args.dump_frames:
                    dump_f32(dump_dir, frame_stage(pending_hidden_after_step_frame, "hidden_after_step"),
                             outputs.last_hidden_state[0, -1, :],
                             "canonical LM hidden state after feeding generated speech embedding")
                pending_hidden_after_step_frame = None
                if args.stop_after == "first_diffusion" and generated_diffusion_frames >= max(1, args.dump_frames):
                    break
            ...
            if diffusion_mask.any():
                ...
                frame_index = generated_diffusion_frames
                should_dump_frame = frame_index < args.dump_frames
                if should_dump_frame:
                    dump_f32(dump_dir, frame_stage(frame_index, "logits"), logits[0],
                             "canonical per-frame constrained/penalized logits before speech-token selection")
                    dump_i32(dump_dir, frame_stage(frame_index, "selected_token"), next_tokens.to(dtype=torch.int32),
                             "canonical per-frame selected speech-path token")
                    dump_f32(dump_dir, frame_stage(frame_index, "diffusion_cond_pos"), condition[0],
                             "canonical per-frame positive conditioning vector for diffusion sample")
                    dump_f32(dump_dir, frame_stage(frame_index, "diffusion_cond_neg"), neg_condition[0],
                             "canonical per-frame negative conditioning vector for diffusion sample")

                speech_latents, initial_noise = sample_speech_tokens_with_dump(
                    model, condition, neg_condition, args.cfg_scale,
                    dump_dir if should_dump_frame else None,
                    frame_index if should_dump_frame else None,
                )
                if should_dump_frame:
                    dump_f32(dump_dir, frame_stage(frame_index, "diffusion_noise"), initial_noise[0],
                             "canonical per-frame initial Gaussian diffusion noise before denoising")
                    dump_f32(dump_dir, frame_stage(frame_index, "diffusion_latent"), speech_latents[0],
                             "canonical per-frame diffusion-sampled latent before decode unscale")
                ...
                if not first_diffusion_done:
                    ...
                    dump_f32(dump_dir, "19_first_diffusion_noise", initial_noise[0],
                             "canonical first diffusion initial noise before denoising")
                    dump_f32(dump_dir, "20_first_diffusion_latent", speech_latents[0],
                ...
                if should_dump_frame:
                    dump_f32(dump_dir, frame_stage(frame_index, "step_embed"), diffusion_embeds[0],
                             "canonical per-frame next-step embedding produced from diffusion latent")
                if not first_diffusion_done:
                    dump_f32(dump_dir, "21_first_step_embed", diffusion_embeds[0],
                             "canonical next-step embedding produced from first diffusion latent")
                next_inputs_embeds = diffusion_embeds.unsqueeze(1)
                first_diffusion_done = True
                generated_diffusion_frames += 1
                pending_hidden_after_step_frame = frame_index
