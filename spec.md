# Specification: `kugelaudio.cpp` ggml Port
...
The first milestone should be a **proof-of-concept KugelAudio port** that can run **the published open checkpoint `kugelaudio/kugelaudio-0-open` end-to-end** in the `kugelaudio.cpp` codebase, with:
...
## Goals
- Make `kugelaudio.cpp` the concrete ggml-based edge-inference path for KugelAudio.
...
A developer converts the supported KugelAudio checkpoint into the `kugelaudio.cpp` format, runs a CLI call, and gets generated speech end-to-end.
...
### High-level approach
Use the inherited `vibevoice.cpp` code as a migration base and adapt it for KugelAudio.
...
- How much of the existing `kugelaudio.cpp` inference loop can be reused without semantic drift?
